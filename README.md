...
rm yarn.lock && rm yarn-error.log
...
5. - For iOS
     ```bash
     # common error some module can not be found

      watchman watch-del-all
      rm -rf node_modules && yarn install
      `yarn start --reset-cache` then ctrl+C, to remove transform code
      rm -rf /tmp/metro-*

     # update native code
     1. To build ***app*** from native code, Lauch ios emulator

        cd ios && pod install && cd ..

        react-native run-ios

     2. Run ***js*** server to watch code

        yarn start --reset-cache

     3. Waiting 5s for bundle js triggered in console after hit refresh in app simulator

     # for latest stream-chat-react-native
     yarn add react-native-image-picker
     react-native link react-native-image-picker
     pod install

     `yarn react-native run-ios` will re-build app

      # force to update or https://stackoverflow.com/questions/62928493/getstream-chat-error-animatedvalue-attempting-to-set-value-to-undefined
